...
export default function EditarCondicaoPage() {
  const params = useParams()
  const router = useRouter()
  const utenteId = params.id as string
  const condicaoId = params.condicaoId as string
  ...
  const loadCondicao = async () => {
    setLoadingData(true)
    try {
      const res = await fetch(`/api/utentes/${utenteId}/condicoes/${condicaoId}`)
      if (res.ok) {
        const data = await res.json()
        form.setValues({
          artigoId: data.artigoId || '',
          precoCliente: parseFloat(data.precoCliente.toString()),
          valorClinica: parseFloat(data.valorClinica.toString()),
          valorTerapeuta: parseFloat(data.valorTerapeuta.toString()),
          retencaoIRS: parseFloat(data.retencaoIRS.toString()),
          necessitaRecibo: data.necessitaRecibo,
          inicioVigencia: new Date(data.inicioVigencia),
          fimVigencia: data.fimVigencia ? new Date(data.fimVigencia) : null,
        })
      } else {
        notifications.show({
          title: 'Erro',
      ...
      notifications.show({
        title: 'Erro',
        message: 'Erro ao carregar condição',
        color: 'red',
      })
    } finally {
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        notifications.show({
          title: 'Erro',
          message: error.error || error.details?.[0]?.message || 'Erro ao atualizar condição comercial',
          color: 'red',
        })
      }
    } catch (error) {
  ...
  return (
    <Container size="lg" py="md">
      <Title mb="md" order={2}>Editar Condição Comercial</Title>
